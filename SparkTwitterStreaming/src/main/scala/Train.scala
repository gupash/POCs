...
object Train {
  def main(args: Array[String]) {
    ...
    val sparkConf = new SparkConf().setAppName("Twitter Sentiment Analyzer").set("spark.serializer", "org.apache.spark.serializer.KryoSerializer").setMaster("local")
    val sc = new SparkContext(sparkConf)
    val stopWords = sc.broadcast(loadStopWords("file:///Users/ashish/IdeaProjects/POCs/SparkTwitterStreaming/src/main/resources/stopwords.txt")).value
    val allData = sc.textFile("file:///Users/ashish/IdeaProjects/POCs/SparkTwitterStreaming/src/main/resources/DataSample.txt")
    val header = allData.first()
    val data = allData.filter(x => x != header)
    val splits = data.randomSplit(Array(0.4, 0.6), seed = 11L)
